...
from key_phrases import PLAN_ADMIN_PHRASES, find_question_context, extract_selections
...
class DocumentSectionExtractor:
    ...
    def extract_section_with_phrases(self, text: str, section_key: str) -> Tuple[Optional[str], Dict]:
        """Extract section with key phrase processing"""
        try:
            # First extract the section normally
            extracted_text = self.extract_section(text, section_key)
            if not extracted_text:
                return None, {}

            # Process with key phrases if available
            phrase_info = {}
            if section_key in PLAN_ADMIN_PHRASES:
                for question_type, pattern in PLAN_ADMIN_PHRASES[section_key].items():
                    # Find question context
                    context = find_question_context(extracted_text, pattern['phrases'])
                    if context['found']:
                        # Extract selections and values
                        selections = extract_selections(context['question_text'], pattern)
                        phrase_info[question_type] = selections

            # Process text with beautifier using phrases
            processed_text = self.beautifier.process_question_with_phrases(
                extracted_text, 
                section_key
            )

            return processed_text, phrase_info

        except Exception as e:
            error_msg = f"Error extracting section with phrases {section_key}: {str(e)}"
    ...
    def extract_all_sections(self, text: str) -> Dict[str, Union[str, Dict]]:
        """Extract and beautify all sections with phrase processing"""
        results = {}
        
        for section_key in self.sections.keys():
            try:
                extracted_text, phrase_info = self.extract_section_with_phrases(text, section_key)
                if extracted_text:
                    results[section_key] = {
                        'text': extracted_text,
                        'phrases': phrase_info
